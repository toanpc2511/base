...
export class UpdateCustomersStatusModalComponent implements OnInit, OnDestroy {
  ...
  constructor(private tableDemoService: TableDemoService, public modal: NgbActiveModal) {}
  ...
  loadCustomers() {
    const sb = this.tableDemoService.items$.pipe(first()).subscribe((res: Customer[]) => {
  ...
  updateCustomersStatus() {
    this.isLoading = true;
    const sb = this.tableDemoService
      .updateStatusForItems(this.ids, +this.status)
      .pipe(
